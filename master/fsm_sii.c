 ...
 *
 * Used to calculate timeouts based on the jiffies counter.
 *
 ...
 *
 * \return 1 while the FSM is still in progress, 0 once it reached end/error.
 */
int ec_fsm_sii_exec(ec_fsm_sii_t *fsm, /**< finite state machine */
                    ec_datagram_t *datagram /**< datagram to use */
                    )
{
    if (fsm->state == ec_fsm_sii_state_end
    ...
    if (fsm->datagram &&
        ...
        // previously queued datagram not yet received; keep waiting
...
/** Returns non-zero if the state machine terminated with success. */
        ...
        )
{
    ...
    EC_WRITE_U8 (datagram->data,     0x81); /* two address octets + enable
                                               write access */
        ...
        )
{
    ...
    fsm->read_word_count = EC_READ_U8(fsm->datagram->data) & 0x40 ? 4 : 2;
    if (fsm->read_word_count > fsm->slave->sii_parallel_words) {
        fsm->slave->sii_parallel_words = fsm->read_word_count;
    }

    memcpy(fsm->value, fsm->datagram->data + 6, fsm->read_word_count * 2);
        ...
        )
{
    ...
    /* Some slaves never set the busy flag on write completion; wait a
     * few ms before evaluating the status. */
    diff_ms = (fsm->datagram->jiffies_received - fsm->jiffies_start)
            * 1000 / HZ;
